...
import { programTimeline } from "@/content/homeDesign";
  ...
  return (
    <HomeSection sectionId="timeline" variant="light" className="border-b border-border">
      <EditorialContainer narrow>
        <PlateKicker className="mb-[clamp(1.75rem,4vh,2.75rem)]">Fig. 04 — Chronology of a program</PlateKicker>
        <SectionTitle id="timeline-heading" className="sr-only">Chronology of a program</SectionTitle>
        <Reveal className="grid border-t-2 border-t-ink [grid-template-columns:repeat(auto-fit,minmax(150px,1fr))]">
          {programTimeline.map((item, i) => (
            ...
            >
              <div className={cn("mb-3 font-mono text-sm", item.featured ? "text-oxblood" : "text-ink")}>{item.year}</div>
              <h3 className="font-serif text-[17px] font-normal leading-tight">
                {item.italic ? <em>{item.title}</em> : item.title}
              </h3>
